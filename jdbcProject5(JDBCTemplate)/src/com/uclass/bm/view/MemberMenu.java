...
public class MemberMenu {
	...
	public void joinMenu() {
		...
		String m_password = sc.nextLine();
		System.out.print("전화번호 : ");
		String m_tell = sc.nextLine();

		member = new Member();
		member.setM_id(m_id);
		member.setM_password(m_password);
		member.setM_tell(m_tell);

		resultMap = mc.insertMember(member);
	...
	public Member LoginMenu() {
		...
		do {
			...
			resultMap = mc.selectLogin(m_id, m_password);

			if ((boolean) resultMap.get("isSuccess")) {
				m = (Member) resultMap.get("res");
			}
